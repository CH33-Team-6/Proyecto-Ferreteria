...
CREATE TABLE IF NOT EXISTS `preciojustodb`.`usuario` (
  ...
  CONSTRAINT `fk_usuario_carrito-compras1`
    FOREIGN KEY (`carrito-compras_idcarrito-compras`)
    REFERENCES `preciojustodb`.`carrito-compras` (`idcarrito-compras`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `preciojustodb`.`orden-de-compra` (
  ...
  CONSTRAINT `fk_orden-de-compra_sucursal1`
    FOREIGN KEY (`sucursal_idsucursal`)
    REFERENCES `preciojustodb`.`sucursal` (`idsucursal`),
  CONSTRAINT `fk_orden-de-compra_usuario1`
    FOREIGN KEY (`usuario_idusuario`)
    REFERENCES `preciojustodb`.`usuario` (`idusuario`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `preciojustodb`.`productos` (
  ...
  CONSTRAINT `fk_productos_carrito-compras1`
    FOREIGN KEY (`carrito-compras_idcarrito-compras`)
    REFERENCES `preciojustodb`.`carrito-compras` (`idcarrito-compras`),
  CONSTRAINT `fk_productos_categorias`
    FOREIGN KEY (`categorias_idcategorias`)
    REFERENCES `preciojustodb`.`categorias` (`idcategorias`),
  CONSTRAINT `fk_productos_sucursal1`
    FOREIGN KEY (`sucursal_idsucursal`)
    REFERENCES `preciojustodb`.`sucursal` (`idsucursal`),
  CONSTRAINT `fk_productos_orden-de-compra1`
    FOREIGN KEY (`orden-de-compra_idorden-de-compra` , `orden-de-compra_usuario_idusuario` , `orden-de-compra_sucursal_idsucursal`)
    REFERENCES `preciojustodb`.`orden-de-compra` (`idorden-de-compra` , `usuario_idusuario` , `sucursal_idsucursal`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
